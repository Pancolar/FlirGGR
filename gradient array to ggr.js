...
var text = "GIMP Gradient\nName: Ironbow\n"+String(colors.length-1);
function left(i) {
  return (1.0/(colors.length - 1)*i);
}
function middle(i) {
  return (1.0/(colors.length - 1)*(i+0.5));
}
function right(i) {
  return (1.0/(colors.length - 1)*(i+1));
}
console.log(text)
function setup() {
  for (let i = 0; i < colors.length-1; i++) {
    rgb1 = hexToRGB01(colors[i]);
    rgb2 = hexToRGB01(colors[i+1]);

    text += left(i).toFixed(6) + " " + middle(i).toFixed(6) + " " + right(i).toFixed(6) + " ";
    text += (rgb1.r).toFixed(6) + " " + (rgb1.g).toFixed(6) + " " + (rgb1.b).toFixed(6) + " 1.000000 " 
    text += (rgb2.r).toFixed(6) + " " + (rgb2.g).toFixed(6) + " " + (rgb2.b).toFixed(6) + " 1.000000 ";
    text += "0 0 0 0"
    text += "\n";
  }
  console.log(text);
  console.log(typeof left(2), left(2));
  console.log(typeof middle(2), middle(2));
  console.log(typeof right(2), right(2));
}
